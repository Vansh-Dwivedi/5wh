...
const createNews = async (req, res) => {
  try {
    ...
    // If scheduling
    if (req.body.scheduledAt && news.status === 'scheduled') {
      news.scheduledAt = new Date(req.body.scheduledAt);
    }
    await news.save();
    recordAudit({
      req,
      action: news.status === 'scheduled' ? 'schedule_news' : 'create_news',
      entityType: 'News',
      entityId: news._id.toString(),
      summary: `${news.status === 'scheduled' ? 'Scheduled' : 'Created'} news '${news.title}'`,
      diff: { after: news.toObject() }
    });
...
const updateNews = async (req, res) => {
  try {
    ...
    // Check authorization - admin and editor can edit all content
    if (req.user.role !== 'admin' && req.user.role !== 'editor' && news.author.toString() !== req.user._id.toString()) {
    ...
    if (req.body.scheduledAt && (updateData.status === 'scheduled' || news.status === 'scheduled')) {
    ...
    recordAudit({
      req,
      action: updateData.status === 'scheduled' ? 'reschedule_news' : 'update_news',
      entityType: 'News',
      entityId: updatedNews._id.toString(),
      summary: `Updated news '${updatedNews.title}'`,
      diff: { before: news.toObject(), after: updatedNews.toObject() }
    });
...
const deleteNews = async (req, res) => {
  try {
    ...
    recordAudit({
      ...
      entityType: 'News',
      entityId: news._id.toString(),
      summary: `Deleted news '${news.title}'`,
      diff: { before: news.toObject() }
    });
