...
const createPodcast = async (req, res) => {
  try {
    ...
    if (req.body.scheduledAt && podcastData.status === 'scheduled') {
      podcastData.scheduledAt = new Date(req.body.scheduledAt);
    }
    const podcast = new Podcast(podcastData);
    await podcast.save();
    recordAudit({
      req,
      action: podcast.status === 'scheduled' ? 'schedule_podcast' : 'create_podcast',
      entityType: 'Podcast',
      entityId: podcast._id.toString(),
      summary: `${podcast.status === 'scheduled' ? 'Scheduled' : 'Created'} podcast '${podcast.title}'`,
      diff: { after: podcast.toObject() }
    });
...
const updatePodcast = async (req, res) => {
  try {
    ...
    if (req.user.role !== 'admin' && req.user.role !== 'editor' && podcast.author.toString() !== req.user._id.toString()) {
    ...
    if (req.body.scheduledAt && (updateData.status === 'scheduled' || podcast.status === 'scheduled')) {
    ...
    recordAudit({
      req,
      action: updateData.status === 'scheduled' ? 'reschedule_podcast' : 'update_podcast',
      entityType: 'Podcast',
      entityId: updatedPodcast._id.toString(),
      summary: `Updated podcast '${updatedPodcast.title}'`,
      diff: { before: podcast.toObject(), after: updatedPodcast.toObject() }
    });
...
const deletePodcast = async (req, res) => {
  try {
    ...
    recordAudit({
      ...
      entityType: 'Podcast',
      entityId: podcast._id.toString(),
      summary: `Deleted podcast '${podcast.title}'`,
      diff: { before: podcast.toObject() }
    });
...
module.exports = {
  ...
  deletePodcast,
  getAdminPodcasts,
  // Added below; ensure export after definition
  getPodcastById: async (req, res) => {
    try {
      ...
      if (e.name === 'CastError') return res.status(400).json({ message: 'Invalid podcast id' });
      console.error('Get podcast by id error:', e);
      res.status(500).json({ message: 'Server error' });
    }
  }
};