...
const AdvertisingSidebar = ({ placement = 'right', sx = {} }) => {
  ...
  const fetchAdvertisers = async () => {
    try {
      const url = `http://ec2-16-52-123-203.ca-central-1.compute.amazonaws.com:5000/api/advertisers?active=true&adType=sidebar&placement=${placement}`;
  ...
  const handleAdvertiserClick = async (advertiser) => {
    try {
      // Track click
      await fetch(`http://ec2-16-52-123-203.ca-central-1.compute.amazonaws.com:5000/api/advertisers/${advertiser._id}/click`, {
        method: 'POST'
      });
  ...
  const handleAdvertiserImpression = async (advertiserId) => {
    try {
      await fetch(`http://ec2-16-52-123-203.ca-central-1.compute.amazonaws.com:5000/api/advertisers/${advertiserId}/impression`, {
        method: 'POST'
      });
    } catch (error) {
                ...
                <Box
                  ...
                  src={
                    advertiser.logo.url.startsWith('http') 
                      ? advertiser.logo.url 
                      : `http://ec2-16-52-123-203.ca-central-1.compute.amazonaws.com:5000${advertiser.logo.url}`
                  }
