...
const CreatePodcast = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.status === 'scheduled') {
      if (!formData.scheduledAt) {
        setError('Scheduled time is required for scheduled status');
        return;
      }
  ...
  const successMsg = formData.status === 'scheduled' ? 'Podcast scheduled successfully!' : 'Podcast created successfully!';
  ...
  const msg = err.response?.data?.message || 'Failed to create podcast';
  setError(msg);
  toast.error(msg);
                    ...
                    {formData.status === 'scheduled' && (
                      <Grid item xs={12}>
                        <TextField
                          ...
                          helperText="When to auto-publish this podcast"
                        />
                      </Grid>
                    )}
