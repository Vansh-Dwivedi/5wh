  ...
  const handlePublishNow = async (item) => {
    try {
      ...
      switch(item.type){
        ...
        case 'Opinion': updater = async(id)=> fetch(`http://5whmedia.com/api/opinions/${id}`, { method:'PUT', headers:{ 'Authorization':`Bearer ${localStorage.getItem('token')}` }, body: ( ()=>{ const fd = new FormData(); fd.append('status','published'); return fd; })()}); break;
        default: return;
      }
  ...
  const applyReschedule = async () => {
    ...
    try {
      ...
      switch(rescheduleItem.type){
        ...
        case 'Opinion': updater = (id)=> fetch(`http://5whmedia.com/api/opinions/${id}`, { method:'PUT', headers:{ 'Authorization':`Bearer ${localStorage.getItem('token')}` }, body:(()=>{ const fd=new FormData(); fd.append('status','scheduled'); fd.append('scheduledAt', iso); return fd; })()}); break;
        default: return;
      }
